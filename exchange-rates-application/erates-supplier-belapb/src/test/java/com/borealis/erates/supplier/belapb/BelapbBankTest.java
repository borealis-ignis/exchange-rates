...
import com.borealis.erates.supplier.belapb.BelapbBank;
...
@ActiveProfiles("test")
public class BelapbBankTest {
	
	@Autowired
	private BelapbBank belapbBank;
	
	@Test
	public void belapbBankDataTest() {
		final Bank belapbBankData = (Bank) belapbBank;
		
		assertThat(belapbBankData.getBankCode()).isEqualTo("belapb");
		assertThat(belapbBankData.getBankName()).isEqualTo("BelAgroProm Bank");
	}
	
	@Test
	public void belapbBankSendRequestTest() throws RatesProcessingException {
		final HttpResponse response = belapbBank.sendRequest();
	...
	@Test
	public void belapbBankParseTest() throws RatesProcessingException {
		...
		final String content = TestIOUtil.readFile("com/borealis/erates/supplier/belapb/parser/correct_response.html");
		
		final List<ExchangeRateDto> rates = belapbBank.parse(content, currencies);
