...
import com.borealis.erates.supplier.bsb.parser.BelswissExchangeRatesParser;
 ...
 */
@Component
public class BelswissBank extends AbstractBankProcessor implements Bank {
	...
	private BelswissExchangeRatesParser parser;
	
	public BelswissBank(
			@Value("${bank.bsb.name:}") final String bankName,
			@Value("${bank.bsb.code:}") final String bankCode,
			@Value("${bank.bsb.rates.url:}") final String url,
			final HttpTransport transport,
			final BelswissExchangeRatesParser parser) {
		this.name = bankName;
		this.code = bankCode;
		this.url = url;
